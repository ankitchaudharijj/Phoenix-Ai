...
const PLANS = [
  { name: "Free", price: "₹0", period: "forever", color: T.stoneGray, features: ["3 scans/month", "URL scanner only", "Basic report", "Community support"], limits: { scans: 3, scanners: ["url"] } },
  { name: "Pro", price: "₹499", period: "/month", color: T.terracotta, popular: true, features: ["Unlimited scans", "All 6 scanner types", "Full PDF reports", "AI explanation", "Priority support", "Verified by Phoenix AI"], limits: { scans: -1, scanners: ["url","code","zip","api","threat","live"] } },
  { name: "Enterprise", price: "₹4,999", period: "/month", color: T.charcoalWarm, features: ["Everything in Pro", "Team access (5 users)", "API access", "Dedicated support", "Custom branding"], limits: { scans: -1, scanners: ["url","code","zip","api","threat","live"] } }
];
...
export default function App() {
  ...
  const getPlanObj = (planName) => {
    ...
    if (n === "PRO") return PLANS[1];
    if (n === "ENTERPRISE") return PLANS[2];
    return PLANS[0];
  };
  const canUseScanType = (tab) => getPlanObj(userPlan).limits.scanners.includes(tab);
  const isLimitReached = () => {
    const n = userPlan?.toUpperCase();
    if (n === "PRO" || n === "ENTERPRISE") return false;
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (res.ok && data.message === "OTP_SENT") {
        ...
        setPage("verify-otp");
        setAuthMsg("");
      } else setAuthMsg("❌ " + (data.detail || "Registration failed"));
    } catch { setAuthMsg("❌ Server error!"); }
  };

  const handleVerifyOTP = async () => {
    if (!otp || otp.length !== 6) return setOtpMsg("6-digit OTP daalo!");
    setOtpLoading(true);
    try {
      const res = await fetch(`${API}/verify-otp`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: pendingEmail, otp }) });
      const data = await res.json();
      if (res.ok) {
        setUser(data); setUserPlan(data.plan || "Free");
        setMonthlyScans(data.monthly_scan_count || 0);
        setPage("dashboard"); setOtpMsg("");
      } else setOtpMsg("❌ " + data.detail);
    } catch { setOtpMsg("❌ Server error!"); }
    setOtpLoading(false);
  };

  const handleResendOTP = async () => {
    try {
      const res = await fetch(`${API}/resend-otp`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: pendingEmail }) });
      if (res.ok) setOtpMsg("✅ New OTP sent!");
      else setOtpMsg("❌ Failed to resend");
    } catch { setOtpMsg("❌ Server error!"); }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      } else if (res.status === 403 && data.detail === "EMAIL_NOT_VERIFIED") {
        setPendingEmail(email);
        await fetch(`${API}/resend-otp`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) });
        setPage("verify-otp");
        setAuthMsg("");
  ...
  const submitScan = async () => {
    ...
    if (!canUseScanType(scanTab)) { setUpgradeReason("scanner"); setShowUpgrade(true); return; }
    if (isLimitReached()) { setUpgradeReason("limit"); setShowUpgrade(true); return; }
    ...
    try {
      ...
      if (scanTab === "zip") {
        ...
        if (res.status === 429 || res.status === 403) { setUpgradeReason(res.status === 429 ? "limit" : "scanner"); setShowUpgrade(true); setLoading(false); return; }
        data = await res.json();
      }
      setLoadingProgress(100);
      setTimeout(async () => {
        setResult(data.result); setScanId(data.scan_id); setLoading(false);
        if (user?.user_id) {
          try {
  ...
  const loadTeamMembers = async () => {
    ...
    try { const res = await fetch(`${API}/team/members/${user.user_id}`); const data = await res.json(); setTeamMembers(data.members || []); } catch {}
  };

  const inviteTeamMember = async () => {
    if (!inviteEmail) return setInviteMsg("Email daalo!");
    try {
      const res = await fetch(`${API}/team/invite`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ owner_id: user.user_id, member_email: inviteEmail }) });
      const data = await res.json();
      if (res.ok) { setInviteMsg("✅ Invitation sent!"); setInviteEmail(""); loadTeamMembers(); }
      else setInviteMsg("❌ " + data.detail);
  ...
  const removeTeamMember = async (memberId) => {
    ...
    try {
      await fetch(`${API}/team/remove/${memberId}?owner_id=${user.user_id}`, { method: "DELETE" });
      loadTeamMembers();
    } catch {}
  };

  const loadApiKey = async () => {
    try { const res = await fetch(`${API}/api-key/${user.user_id}`); const data = await res.json(); setApiKey(data.api_key); } catch {}
  };

  const generateApiKey = async () => {
    setApiKeyLoading(true);
    try {
      const res = await fetch(`${API}/api-key/generate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: user.user_id }) });
      const data = await res.json();
  ...
  const deleteUser = async (userId, userEmail) => {
    if (!window.confirm(`Delete ${userEmail}? Cannot be undone!`)) return;
    try {
      const res = await fetch(`${API}/admin/delete-user?user_id=${userId}&secret=${ADMIN_SECRET}`, { method: "DELETE" });
      if (res.ok) { alert("✅ Deleted!"); loadAdminUsers(); }
    } catch {}
  };
  ...
  // ── UPGRADE MODAL ──
  const UpgradeModal = () => (
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,20,19,0.92)", backdropFilter: "blur(8px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px", minHeight: "100vh", minWidth: "100vw", boxSizing: "border-box" }}>
      <div style={{ background: T.ivory, borderRadius: 20, padding: "40px 32px", width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto", textAlign: "center", boxShadow: "rgba(0,0,0,0.3) 0px 24px 64px", boxSizing: "border-box" }}>
        <div style={{ width: 56, height: 56, borderRadius: "50%", background: T.warmSand, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", fontSize: 24 }}>🔒</div>
        <div style={{ fontSize: 11, color: T.terracotta, letterSpacing: "0.5px", textTransform: "uppercase", marginBottom: 10 }}>
          {upgradeReason === "limit" ? "Monthly limit reached" : "Feature locked"}
        </div>
        <h2 style={{ fontSize: 24, fontWeight: 500, color: T.nearBlack, margin: "0 0 10px", fontFamily: "Georgia, serif", lineHeight: 1.2 }}>
          {upgradeReason === "limit" ? "You've used all 3 free scans" : "This scanner requires Pro"}
        </h2>
        <p style={{ fontSize: 14, color: T.oliveGray, margin: "0 0 24px", lineHeight: 1.6 }}>
          {upgradeReason === "limit" ? "Upgrade to Pro for unlimited scans and all scanner types." : "Upgrade to Pro to access all 6 scanner types."}
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
          {[{ name: "Pro", price: "₹499/mo", color: T.terracotta, features: ["Unlimited scans","All 6 scanners","Full PDF reports","AI analysis"] },
            { name: "Enterprise", price: "₹4,999/mo", color: T.charcoalWarm, features: ["Everything in Pro","5 team members","API access","Dedicated support"] }].map(plan => (
            <div key={plan.name} style={{ background: T.parchment, border: `1px solid ${T.borderWarm}`, borderRadius: 12, padding: 16, textAlign: "left" }}>
              <div style={{ fontSize: 15, fontWeight: 500, color: plan.color, fontFamily: "Georgia, serif", marginBottom: 3 }}>{plan.name}</div>
              <div style={{ fontSize: 12, color: T.stoneGray, marginBottom: 10 }}>{plan.price}</div>
              {plan.features.map((f, i) => <div key={i} style={{ display: "flex", gap: 6, marginBottom: 5, fontSize: 12, color: T.oliveGray }}><span style={{ color: T.terracotta }}>✓</span>{f}</div>)}
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => setShowUpgrade(false)} style={{ padding: "10px 20px", background: T.warmSand, border: `1px solid ${T.borderWarm}`, borderRadius: 10, color: T.charcoalWarm, cursor: "pointer", fontSize: 14, fontFamily: "inherit" }}>Maybe later</button>
          <button onClick={() => { setShowUpgrade(false); setActivePage("plans"); }} style={{ padding: "10px 24px", background: T.terracotta, border: "none", borderRadius: 10, color: T.ivory, cursor: "pointer", fontSize: 14, fontFamily: "Georgia, serif", fontWeight: 500 }}>Upgrade now →</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", minHeight: "100vh" }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 72px", borderRight: `1px solid ${T.borderDark}` }}>
          <div style={{ opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(24px)", transition: "all 0.7s ease" }}>
            ...
            <h1 style={{ fontSize: 64, fontWeight: 500, lineHeight: 1.1, letterSpacing: "-1px", color: T.ivory, margin: 0, marginBottom: 24 }}>Phoenix <span style={{ color: T.terracotta }}>AI</span></h1>
            <p style={{ fontSize: 20, color: T.warmSilver, lineHeight: 1.6, margin: 0, marginBottom: 56, maxWidth: 440, fontFamily: "system-ui" }}>AI-powered security testing that finds vulnerabilities in minutes, not weeks.</p>
        ...
        <div style={{ width: 480, display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 56px", background: "rgba(20,20,19,0.9)", backdropFilter: "blur(20px)" }}>
          <div style={{ opacity: mounted ? 1 : 0, transition: "all 0.6s ease 0.2s" }}>
            <h2 style={{ fontSize: 32, fontWeight: 500, color: T.ivory, margin: 0, marginBottom: 8, fontFamily: "Georgia, serif" }}>Welcome back</h2>
            <p style={{ fontSize: 15, color: T.stoneGray, marginBottom: 40, fontFamily: "system-ui" }}>Sign in to Phoenix AI</p>
            ...
            <button onClick={handleLogin} style={{ width: "100%", padding: "13px 20px", background: T.terracotta, border: "none", borderRadius: 12, color: T.ivory, fontSize: 15, fontWeight: 500, cursor: "pointer", fontFamily: "Georgia, serif", transition: "all 0.2s" }}>Sign in →</button>
            <div style={{ textAlign: "center", marginTop: 20, fontSize: 14, color: T.stoneGray, fontFamily: "system-ui" }}>
              Don't have an account? <span style={{ color: T.coral, cursor: "pointer" }} onClick={() => { setPage("register"); setAuthMsg(""); }}>Create one</span>
            </div>
  ...
  if (page === "register") return (
    <div style={{ minHeight: "100vh", background: T.nearBlack, fontFamily: "Georgia, serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", minHeight: "100vh" }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 72px", borderRight: `1px solid ${T.borderDark}` }}>
          <div style={{ opacity: mounted ? 1 : 0, transition: "all 0.7s ease" }}>
            <div style={{ fontSize: 13, fontFamily: "system-ui", color: T.terracotta, letterSpacing: "0.5px", marginBottom: 24, textTransform: "uppercase" }}>AI Security Platform</div>
            <h1 style={{ fontSize: 64, fontWeight: 500, lineHeight: 1.1, letterSpacing: "-1px", color: T.ivory, margin: 0, marginBottom: 24 }}>Phoenix <span style={{ color: T.terracotta }}>AI</span></h1>
            <p style={{ fontSize: 20, color: T.warmSilver, lineHeight: 1.6, margin: 0, maxWidth: 440, fontFamily: "system-ui" }}>Join security professionals. Scan smarter, not harder.</p>
          </div>
        </div>
        <div style={{ width: 480, display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 56px", background: "rgba(20,20,19,0.9)", backdropFilter: "blur(20px)" }}>
          <h2 style={{ fontSize: 32, fontWeight: 500, color: T.ivory, margin: 0, marginBottom: 8, fontFamily: "Georgia, serif" }}>Create account</h2>
          <p style={{ fontSize: 15, color: T.stoneGray, marginBottom: 40, fontFamily: "system-ui" }}>Start your free security assessment</p>
          <label style={{ ...labelStyle, color: T.stoneGray }}>Email address</label>
          <input style={darkInputStyle} placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} />
          <label style={{ ...labelStyle, color: T.stoneGray }}>Password</label>
          <input style={darkInputStyle} placeholder="••••••••" type="password" value={password} onChange={e => setPassword(e.target.value)} />
          {authMsg && <div style={{ fontSize: 13, color: T.errorCrimson, marginBottom: 16, padding: "10px 14px", background: "rgba(181,51,51,0.08)", borderRadius: 8, fontFamily: "system-ui" }}>{authMsg}</div>}
          <button onClick={handleRegister} style={{ width: "100%", padding: "13px 20px", background: T.terracotta, border: "none", borderRadius: 12, color: T.ivory, fontSize: 15, fontWeight: 500, cursor: "pointer", fontFamily: "Georgia, serif" }}>Create account →</button>
          <div style={{ textAlign: "center", marginTop: 20, fontSize: 14, color: T.stoneGray, fontFamily: "system-ui" }}>
            Already have an account? <span style={{ color: T.coral, cursor: "pointer" }} onClick={() => { setPage("login"); setAuthMsg(""); }}>Sign in</span>
          </div>
        </div>
      </div>
    </div>
  ...
  if (page === "verify-otp") return (
    <div style={{ minHeight: "100vh", background: T.nearBlack, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Georgia, serif", position: "relative" }}>
      <Particles />
      <div style={{ position: "relative", zIndex: 1, width: 440, background: "rgba(20,20,19,0.95)", border: `1px solid ${T.borderDark}`, borderRadius: 24, padding: "48px 40px", textAlign: "center" }}>
        <div style={{ width: 64, height: 64, borderRadius: "50%", background: "rgba(201,100,66,0.1)", border: "1px solid rgba(201,100,66,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px", fontSize: 28 }}>📧</div>
        <div style={{ fontSize: 12, color: T.terracotta, letterSpacing: "0.5px", textTransform: "uppercase", marginBottom: 12, fontFamily: "system-ui" }}>Email Verification</div>
        <h2 style={{ fontSize: 28, fontWeight: 500, color: T.ivory, margin: "0 0 8px" }}>Check your inbox</h2>
        <p style={{ fontSize: 14, color: T.stoneGray, marginBottom: 32, fontFamily: "system-ui", lineHeight: 1.6 }}>
          We sent a 6-digit code to<br /><strong style={{ color: T.ivory }}>{pendingEmail}</strong>
        </p>

        <label style={{ ...labelStyle, color: T.stoneGray, textAlign: "left", display: "block" }}>Enter OTP Code</label>
        <input
          style={{ ...darkInputStyle, fontSize: 28, textAlign: "center", letterSpacing: "8px", fontFamily: "Georgia, serif", marginBottom: 8 }}
          placeholder="000000"
          maxLength={6}
          value={otp}
        ...
        {otpMsg && <div style={{ fontSize: 13, color: otpMsg.includes('✅') ? "#2d6a4f" : T.errorCrimson, marginBottom: 16, padding: "10px 14px", background: otpMsg.includes('✅') ? "rgba(45,106,79,0.08)" : "rgba(181,51,51,0.08)", borderRadius: 8, fontFamily: "system-ui" }}>{otpMsg}</div>}

        <button onClick={handleVerifyOTP} disabled={otpLoading}
          style={{ width: "100%", padding: "13px", background: otpLoading ? "rgba(201,100,66,0.3)" : T.terracotta, border: "none", borderRadius: 12, color: T.ivory, fontSize: 15, fontWeight: 500, cursor: otpLoading ? "not-allowed" : "pointer", fontFamily: "Georgia, serif", marginBottom: 16 }}>
          {otpLoading ? "Verifying…" : "Verify Email →"}
        </button>

        <div style={{ fontSize: 13, color: T.stoneGray, fontFamily: "system-ui" }}>
          Didn't receive it?{" "}
          <span style={{ color: T.coral, cursor: "pointer" }} onClick={handleResendOTP}>Resend OTP</span>
        </div>
        <div style={{ marginTop: 16 }}>
          <span style={{ fontSize: 12, color: T.borderDark, cursor: "pointer", fontFamily: "system-ui" }} onClick={() => setPage("login")}>← Back to login</span>
        </div>
      </div>
    </div>
  );
  ...
  // ── ADMIN LOGIN ──
  if (page === "admin" && !adminLoggedIn) return (
    <div style={{ minHeight: "100vh", background: T.nearBlack, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Georgia, serif" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, width: 400, background: "rgba(20,20,19,0.95)", border: `1px solid ${T.borderDark}`, borderRadius: 20, padding: 48 }}>
        ...
        <button onClick={adminLogin} style={{ width: "100%", padding: "13px", background: T.terracotta, border: "none", borderRadius: 12, color: T.ivory, fontSize: 15, fontWeight: 500, cursor: "pointer", fontFamily: "Georgia, serif" }}>Access Dashboard →</button>
        <div style={{ textAlign: "center", marginTop: 16 }}>
  ...
  if (page === "admin" && adminLoggedIn) return (
    <div style={{ minHeight: "100vh", background: T.parchment, fontFamily: "system-ui, sans-serif" }}>
      <nav style={{ background: T.nearBlack, borderBottom: `1px solid ${T.borderDark}`, padding: "0 32px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ fontSize: 20, fontWeight: 500, fontFamily: "Georgia, serif", color: T.ivory }}>Phoenix <span style={{ color: T.terracotta }}>AI</span> <span style={{ fontSize: 11, color: T.stoneGray, marginLeft: 8, letterSpacing: "0.5px", textTransform: "uppercase" }}>Admin</span></div>
        <div style={{ display: "flex", gap: 4 }}>
          {["overview","users","scans"].map(tab => (
            <button key={tab} onClick={() => setAdminTab(tab)} style={{ padding: "8px 16px", background: adminTab === tab ? "rgba(201,100,66,0.15)" : "transparent", border: "none", borderRadius: 8, color: adminTab === tab ? T.terracotta : T.stoneGray, cursor: "pointer", fontSize: 14, fontFamily: "inherit", fontWeight: adminTab === tab ? 500 : 400, textTransform: "capitalize" }}>{tab}</button>
          ))}
        </div>
        <button onClick={() => { setAdminLoggedIn(false); setPage("login"); }} style={{ padding: "7px 16px", background: "transparent", border: `1px solid ${T.borderDark}`, borderRadius: 8, color: T.stoneGray, cursor: "pointer", fontSize: 13 }}>Sign out</button>
      </nav>
        ...
        {adminTab === "users" && (
          <div>
            ...
            <div style={{ background: T.ivory, border: `1px solid ${T.borderCream}`, borderRadius: 16, overflow: "hidden" }}>
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 0.5fr 1fr 1fr 2fr", padding: "12px 24px", background: T.warmSand, borderBottom: `1px solid ${T.borderCream}` }}>
                {["Email","Plan","Verified","Expires","Last Login","Actions"].map(h => (
              ...
              {adminUsers.map((u, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 0.5fr 1fr 1fr 2fr", padding: "16px 24px", borderBottom: i < adminUsers.length-1 ? `1px solid ${T.borderCream}` : "none", alignItems: "center" }}>
                  <div>
                    <div style={{ fontSize: 14, color: T.nearBlack, fontWeight: 500 }}>{u.email}</div>
                    <div style={{ fontSize: 11, color: T.stoneGray, marginTop: 2 }}>ID: {u.id} · {u.monthly_scan_count||0} this month</div>
                  </div>
                  ...
                  <div style={{ fontSize: 12 }}>{u.verified ? "✅" : "❌"}</div>
                  ...
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => deleteUser(u.id, u.email)}
                      style={{ padding: "4px 8px", background: "rgba(181,51,51,0.1)", border: "1px solid rgba(181,51,51,0.3)", borderRadius: 6, color: T.errorCrimson, cursor: "pointer", fontSize: 10, fontFamily: "inherit", fontWeight: 500 }}>Del</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ background: T.ivory, border: `1px solid ${T.borderCream}`, borderRadius: 16, overflow: "hidden" }}>
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1.5fr", padding: "12px 24px", background: T.warmSand, borderBottom: `1px solid ${T.borderCream}` }}>
                {["Target","Type","User","Date"].map(h => <div key={h} style={{ fontSize: 11, color: T.stoneGray, letterSpacing: "0.5px", textTransform: "uppercase", fontWeight: 500 }}>{h}</div>)}
              </div>
              ...
              {adminScans.map((s, i) => (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1.5fr", padding: "14px 24px", borderBottom: i < adminScans.length-1 ? `1px solid ${T.borderCream}` : "none", alignItems: "center" }}>
                  ...
                  <span style={{ fontSize: 11, padding: "2px 8px", background: T.warmSand, border: `1px solid ${T.borderWarm}`, color: T.charcoalWarm, borderRadius: 20, textTransform: "uppercase", display: "inline-block" }}>{s.type}</span>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px", display: "flex", alignItems: "center", height: 60, justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {[{id:"scan",label:"Scanner"},{id:"history",label:"History"},{id:"company",label:"Company"},{id:"plans",label:"Plans"}].map(item => (
              <button key={item.id} onClick={() => { setActivePage(item.id); if(item.id==="history")loadHistory(); if(item.id==="company"&&userPlan?.toUpperCase()==="ENTERPRISE"){loadTeamMembers();loadApiKey();} }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {userPlan?.toUpperCase()==="FREE" && (
              <div style={{ fontSize: 12, color: T.stoneGray, background: T.parchment, border: `1px solid ${T.borderWarm}`, borderRadius: 20, padding: "4px 12px" }}>
                <span style={{ color: monthlyScans>=3?T.errorCrimson:T.terracotta, fontWeight: 500 }}>{monthlyScans}</span>/3 scans
              </div>
            )}
            {userPlan?.toUpperCase()==="PRO" && (
              <div style={{ fontSize: 11, color: T.terracotta, background: "rgba(201,100,66,0.08)", border: "1px solid rgba(201,100,66,0.2)", borderRadius: 20, padding: "4px 12px", fontWeight: 500 }}>✓ Verified</div>
            )}
            ...
            <button onClick={() => { setUser(null); setPage("login"); setResult(null); }} style={{ padding: "7px 16px", background: "transparent", border: `1px solid ${T.borderWarm}`, borderRadius: 8, color: T.oliveGray, cursor: "pointer", fontSize: 14, fontFamily: "inherit" }}>Sign out</button>
          </div>
        </div>
      </nav>
        ...
        {activePage === "scan" && (
          <div>
            <div style={{ marginBottom: 48, borderBottom: `1px solid ${T.borderCream}`, paddingBottom: 40 }}>
              ...
              <h1 style={{ fontSize: 52, fontWeight: 500, color: T.nearBlack, margin: 0, marginBottom: 16, fontFamily: "Georgia, serif", lineHeight: 1.15, letterSpacing: "-1px" }}>Find vulnerabilities<br />before attackers do.</h1>
              <p style={{ fontSize: 18, color: T.oliveGray, margin: 0, lineHeight: 1.6, maxWidth: 520 }}>AI-powered scanning across URLs, code, APIs, and live applications.</p>
            </div>

            {userPlan?.toUpperCase()==="FREE" && monthlyScans>=2 && monthlyScans<3 && (
              <div style={{ background: "rgba(201,100,66,0.06)", border: "1px solid rgba(201,100,66,0.2)", borderRadius: 12, padding: "14px 20px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontSize: 14, color: T.charcoalWarm }}>⚠️ <strong>1 scan remaining</strong> this month.</div>
              ...
              {SCAN_TABS.map(([tab,label]) => {
                ...
                return (
                  <button key={tab} onClick={() => { setScanTab(tab); setResult(null); setScanId(null); }}
                    style={{ flex: 1, padding: "14px 8px", background: scanTab===tab?T.nearBlack:"transparent", border: "none", borderRight: `1px solid ${T.borderWarm}`, color: scanTab===tab?T.ivory:locked?T.warmSand:T.oliveGray, cursor: "pointer", fontSize: 13, fontFamily: "inherit", fontWeight: scanTab===tab?500:400, transition: "all 0.15s", whiteSpace: "nowrap" }}>
                    {label}{locked&&" 🔒"}
                  </button>
                );
              })}
            ...
            {!loading && (
              <div style={{ background: T.ivory, border: `1px solid ${T.borderWarm}`, borderRadius: 16, padding: 36, marginBottom: 36, boxShadow: "rgba(0,0,0,0.03) 0px 4px 24px" }}>
                <div style={{ fontSize: 12, color: T.stoneGray, letterSpacing: "0.5px", textTransform: "uppercase", marginBottom: 20 }}>{SCAN_TABS.find(t=>t[0]===scanTab)?.[2]}</div>

                {(scanTab==="url"||scanTab==="api")&&(<><label style={labelStyle}>{scanTab==="api"?"API Base URL":"Target URL"}</label><input value={url} onChange={e=>setUrl(e.target.value)} placeholder={scanTab==="api"?"https://api.example.com":"https://example.com"} style={{...inputStyle,fontSize:16,marginBottom:0}}/></>)}
                {scanTab==="code"&&(<><label style={labelStyle}>Source Code</label><textarea value={code} onChange={e=>setCode(e.target.value)} placeholder="// Paste source code..." rows={12} style={{...inputStyle,marginBottom:0,resize:"vertical",lineHeight:1.7,fontFamily:"monospace",fontSize:13}}/></>)}
                {scanTab==="zip"&&(<>
                ...
                {scanTab==="threat"&&(<><label style={labelStyle}>Target URL or IP</label><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://example.com or 8.8.8.8" style={{...inputStyle,fontSize:16,marginBottom:12}}/><div style={{padding:"12px 16px",background:T.parchment,border:`1px solid ${T.borderCream}`,borderRadius:10,fontSize:13,color:T.oliveGray}}>AbuseIPDB · CVE Database · Blacklist · SSL · Ports · Subdomains</div></>)}
                {scanTab==="live"&&(<>
                  <label style={labelStyle}>Target App URL</label><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://example.com" style={{...inputStyle,fontSize:16}}/>
                  <label style={labelStyle}>Login URL <span style={{textTransform:"none",fontSize:11}}>(optional)</span></label><input value={loginUrl} onChange={e=>setLoginUrl(e.target.value)} placeholder="https://example.com/login" style={inputStyle}/>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 24, paddingTop: 24, borderTop: `1px solid ${T.borderCream}` }}>
                  {userPlan?.toUpperCase()==="FREE"&&<div style={{fontSize:13,color:T.stoneGray}}>{3-monthlyScans} scan{3-monthlyScans!==1?"s":""} remaining</div>}
                  <div style={{marginLeft:"auto"}}>
                    {isLimitReached()?(
                      <button onClick={()=>{setUpgradeReason("limit");setShowUpgrade(true);}} style={{padding:"12px 32px",background:T.terracotta,border:"none",borderRadius:10,color:T.ivory,fontSize:15,fontWeight:500,cursor:"pointer",fontFamily:"Georgia, serif"}}>Upgrade to scan →</button>
                    ):(
                      <button onClick={submitScan} style={{padding:"12px 32px",background:T.terracotta,border:"none",borderRadius:10,color:T.ivory,fontSize:15,fontWeight:500,cursor:"pointer",fontFamily:"Georgia, serif",transition:"all 0.2s"}}>Start scan →</button>
                    )}
                  </div>
                </div>
              </div>
            ...
            {loading && (
              <div style={{ background: T.ivory, border: `1px solid ${T.borderWarm}`, borderRadius: 16, padding: "48px 40px", marginBottom: 32, textAlign: "center" }}>
                <div style={{ display: "flex", justifyContent: "center", gap: 10, marginBottom: 32 }}>
                  {[0,1,2,3,4].map(i=><div key={i} style={{width:10,height:10,borderRadius:"50%",background:T.terracotta,animation:"bounce 1.2s ease-in-out infinite",animationDelay:`${i*0.15}s`}}/>)}
                </div>
                ...
                <div style={{fontSize:13,color:T.stoneGray}}>{Math.round(loadingProgress)}% complete · 30–60 seconds</div>
              </div>
            )}
              ...
              <div>
                {result.scanned_files?.length>0&&(
                  <div style={{background:T.ivory,border:`1px solid ${T.borderWarm}`,borderRadius:12,padding:20,marginBottom:20}}>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:8}}>{result.scanned_files.map((f,i)=><span key={i} style={{fontSize:12,padding:"3px 10px",background:T.warmSand,border:`1px solid ${T.borderWarm}`,borderRadius:20,color:T.charcoalWarm}}>{f}</span>)}</div>
                  </div>
                )}
                {result.intel&&(
                  <div style={{background:T.nearBlack,border:`1px solid ${T.borderDark}`,borderRadius:16,padding:28,marginBottom:24}}>
                    ...
                    {result.intel.subdomains?.length>0&&(
                      <div style={{borderTop:`1px solid ${T.borderDark}`,paddingTop:16}}>
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>{result.intel.subdomains.map((s,i)=><span key={i} style={{fontSize:12,padding:"4px 12px",background:"rgba(250,249,245,0.06)",border:`1px solid ${T.borderDark}`,borderRadius:20,color:T.warmSilver}}>{s.subdomain} <span style={{color:T.stoneGray,fontSize:10}}>{s.ip}</span></span>)}</div>
                      </div>
                    )}
                  </div>
                )}
                ...
                {userPlan?.toUpperCase()==="FREE"?(
                  <div style={{background:T.parchment,border:`1px solid ${T.borderWarm}`,borderRadius:12,padding:"20px 24px",marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
                    <div><div style={{fontSize:14,color:T.charcoalWarm,fontWeight:500,marginBottom:4}}>Basic Report Available</div><div style={{fontSize:13,color:T.stoneGray}}>Upgrade to Pro for full PDF with AI analysis.</div></div>
                    <div style={{display:"flex",gap:10}}>
                      <button onClick={()=>downloadReport(result,url||"Source Code",scanId)} style={{padding:"8px 16px",background:T.warmSand,border:`1px solid ${T.borderWarm}`,borderRadius:8,color:T.charcoalWarm,cursor:"pointer",fontSize:13,fontFamily:"inherit"}}>Basic Report</button>
                      <button onClick={()=>{setUpgradeReason("limit");setShowUpgrade(true);}} style={{padding:"8px 16px",background:T.terracotta,border:"none",borderRadius:8,color:T.ivory,cursor:"pointer",fontSize:13,fontFamily:"Georgia, serif",fontWeight:500}}>Get Full PDF →</button>
                    </div>
                  </div>
                ):(
                  <button onClick={()=>downloadReport(result,url||zipFile?.name||"Source Code",scanId)} style={{width:"100%",padding:"13px",background:T.warmSand,border:`1px solid ${T.borderWarm}`,borderRadius:10,color:T.charcoalWarm,cursor:"pointer",fontSize:14,fontFamily:"inherit",fontWeight:500,marginBottom:24}}>Download Full PDF Report</button>
                )}

                {result.ai_explanation&&!result.ai_explanation.includes('Error')&&(
                ...
                {(result.findings??[]).map((f,i)=>(
                  <div key={i} style={{background:T.ivory,borderLeft:`3px solid ${sColor(f.severity)}`,borderRadius:"0 12px 12px 0",padding:"20px 24px",marginBottom:12,boxShadow:"rgba(0,0,0,0.03) 0px 2px 8px",border:`1px solid ${T.borderCream}`}}>
                    ...
                    {f.poc&&userPlan?.toUpperCase()!=="FREE"&&<div style={{fontSize:12,color:T.stoneGray,marginTop:10,padding:"8px 12px",background:T.parchment,borderRadius:8,lineHeight:1.6,fontFamily:"monospace",border:`1px solid ${T.borderCream}`}}>{f.poc}</div>}
                    {f.poc&&userPlan?.toUpperCase()==="FREE"&&<div style={{fontSize:12,color:T.warmSilver,marginTop:10,padding:"8px 12px",background:T.warmSand,borderRadius:8,border:`1px solid ${T.borderWarm}`}}>🔒 PoC available on Pro plan</div>}
                  </div>
                ))}
              </div>
            )}
              ...
              <div key={i} style={{background:T.ivory,border:`1px solid ${T.borderCream}`,borderRadius:14,padding:"20px 28px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
                ...
                <div style={{display:"flex",gap:20,alignItems:"center"}}>
                  {scan.result&&(<>
                    ...
                    <button onClick={()=>downloadReport(scan.result,scan.target,scan.id)} style={{padding:"8px 20px",background:T.warmSand,border:`1px solid ${T.borderWarm}`,color:T.charcoalWarm,borderRadius:8,cursor:"pointer",fontSize:13,fontFamily:"inherit",fontWeight:500}}>
                      {userPlan?.toUpperCase()==="FREE"?"Basic Report":"Download PDF"}
                    </button>
                  </>)}
                </div>
              </div>
        ...
        {activePage === "company" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:32}}>
              {[["Total Scans",history.length||0],["Critical Issues",history.filter(s=>(s.result?.high??0)>0).length||0],["Assets",3],["Team",userPlan?.toUpperCase()==="ENTERPRISE"?"Up to 5":1],["Plan",userPlan],["Reports",history.length||0]].map(([l,v])=>(
              ...
              {["https://globalsecurelayerx.in","https://api.globalsecurelayerx.in","https://admin.globalsecurelayerx.in"].map((a,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 0",borderBottom:i<2?`1px solid ${T.borderCream}`:"none"}}>
                  ...
                  <div style={{display:"flex",gap:10}}>
                    ...
                    <button onClick={()=>{setUrl(a);setActivePage("scan");setScanTab("url");}} style={{padding:"7px 16px",background:T.terracotta,border:"none",color:T.ivory,borderRadius:8,cursor:"pointer",fontSize:13,fontFamily:"inherit",fontWeight:500}}>Scan now</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            {userPlan?.toUpperCase()==="ENTERPRISE" && (
              <div style={{background:T.ivory,border:`1px solid ${T.borderCream}`,borderRadius:16,padding:28,marginBottom:20}}>
                ...
                <div style={{fontSize:13,color:T.stoneGray,marginBottom:20}}>Manage your Enterprise team (max 5 members)</div>

                {/* Invite form */}
                <div style={{display:"flex",gap:12,marginBottom:16}}>
                  <input value={inviteEmail} onChange={e=>setInviteEmail(e.target.value)} placeholder="member@example.com" style={{...inputStyle,marginBottom:0,flex:1}}/>
                  <button onClick={inviteTeamMember} style={{padding:"12px 20px",background:T.terracotta,border:"none",borderRadius:10,color:T.ivory,cursor:"pointer",fontSize:14,fontFamily:"Georgia, serif",fontWeight:500,whiteSpace:"nowrap"}}>+ Invite Member</button>
                </div>
                {inviteMsg&&<div style={{fontSize:13,color:inviteMsg.includes("✅")?T.oliveGray:T.errorCrimson,marginBottom:16,padding:"8px 12px",background:inviteMsg.includes("✅")?"rgba(45,106,79,0.08)":"rgba(181,51,51,0.08)",borderRadius:8}}>{inviteMsg}</div>}

                {/* Members list */}
                {teamMembers.length > 0 && (
                  <div style={{border:`1px solid ${T.borderCream}`,borderRadius:10,overflow:"hidden"}}>
                    {teamMembers.map((m,i)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",borderBottom:i<teamMembers.length-1?`1px solid ${T.borderCream}`:"none"}}>
                        <div>
                          <div style={{fontSize:14,color:T.nearBlack,fontWeight:500}}>{m.email}</div>
                          <div style={{fontSize:11,color:T.stoneGray,marginTop:2}}>Invited: {m.invited_at?.slice(0,10)}</div>
                        </div>
                        <div style={{display:"flex",gap:10,alignItems:"center"}}>
                          <span style={{fontSize:11,padding:"3px 10px",background:m.status==="active"?"#f0fdf4":T.warmSand,border:`1px solid ${m.status==="active"?"#bbf7d0":T.borderWarm}`,color:m.status==="active"?"#166534":T.stoneGray,borderRadius:20}}>{m.status}</span>
                          <button onClick={()=>removeTeamMember(m.id)} style={{padding:"5px 12px",background:"rgba(181,51,51,0.08)",border:"1px solid rgba(181,51,51,0.2)",borderRadius:8,color:T.errorCrimson,cursor:"pointer",fontSize:12,fontFamily:"inherit"}}>Remove</button>
                        </div>
                      </div>
                    ))}
                  </div>
                )}
                {teamMembers.length===0&&<div style={{fontSize:13,color:T.stoneGray,padding:"20px 0",textAlign:"center"}}>No team members yet. Invite someone!</div>}
              </div>
            )}
            ...
            {userPlan?.toUpperCase()==="ENTERPRISE" && (
              <div style={{background:T.ivory,border:`1px solid ${T.borderCream}`,borderRadius:16,padding:28}}>
                <div style={{fontSize:16,fontWeight:500,color:T.nearBlack,fontFamily:"Georgia, serif",marginBottom:8}}>API Access</div>
                <div style={{fontSize:13,color:T.stoneGray,marginBottom:20}}>Use Phoenix AI API to integrate security scanning into your workflows.</div>

                {apiKey ? (
                  <div>
                    <div style={{fontSize:11,color:T.stoneGray,letterSpacing:"0.5px",textTransform:"uppercase",marginBottom:8}}>Your API Key</div>
                    <div style={{display:"flex",gap:10,alignItems:"center"}}>
                      <code style={{flex:1,padding:"12px 16px",background:T.parchment,border:`1px solid ${T.borderWarm}`,borderRadius:10,fontSize:13,color:T.charcoalWarm,fontFamily:"monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{apiKey}</code>
                      <button onClick={()=>{navigator.clipboard.writeText(apiKey);alert("✅ Copied!");}} style={{padding:"10px 16px",background:T.warmSand,border:`1px solid ${T.borderWarm}`,borderRadius:8,color:T.charcoalWarm,cursor:"pointer",fontSize:13,fontFamily:"inherit",whiteSpace:"nowrap"}}>Copy</button>
                      <button onClick={generateApiKey} style={{padding:"10px 16px",background:T.warmSand,border:`1px solid ${T.borderWarm}`,borderRadius:8,color:T.charcoalWarm,cursor:"pointer",fontSize:13,fontFamily:"inherit",whiteSpace:"nowrap"}}>Regenerate</button>
                    </div>
                    <div style={{marginTop:12,padding:"12px 16px",background:T.parchment,border:`1px solid ${T.borderCream}`,borderRadius:10,fontSize:12,color:T.stoneGray}}>
                      Use this key in your API requests: <code style={{color:T.charcoalWarm}}>Authorization: Bearer {apiKey?.slice(0,20)}…</code>
                    </div>
                  </div>
                ) : (
                  <button onClick={generateApiKey} disabled={apiKeyLoading} style={{padding:"12px 24px",background:T.terracotta,border:"none",borderRadius:10,color:T.ivory,cursor:"pointer",fontSize:14,fontFamily:"Georgia, serif",fontWeight:500}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:24,marginBottom:48}}>
              {PLANS.map((plan,i)=>(
                <div key={plan.name} style={{background:plan.popular?T.nearBlack:T.ivory,border:`2px solid ${userPlan?.toUpperCase()===plan.name?.toUpperCase()?T.terracotta:plan.popular?T.borderDark:T.borderWarm}`,borderRadius:20,padding:36,position:"relative",opacity:mounted?1:0,transform:mounted?"translateY(0)":"translateY(20px)",transition:`all 0.4s ease ${i*0.08}s`,boxShadow:plan.popular?"rgba(0,0,0,0.12) 0px 8px 32px":"rgba(0,0,0,0.04) 0px 2px 12px"}}>
                  ...
                  {userPlan?.toUpperCase()===plan.name?.toUpperCase()&&<div style={{position:"absolute",top:16,right:16,fontSize:11,color:T.terracotta,fontWeight:500,background:"rgba(201,100,66,0.1)",padding:"3px 10px",borderRadius:20}}>Current</div>}
                  ...
                  <button onClick={()=>{if(userPlan?.toUpperCase()!==plan.name?.toUpperCase())alert(`Contact us to upgrade!\nEmail: securelayerhelp@gmail.com`);}}
                    style={{width:"100%",marginTop:28,padding:"13px",background:userPlan?.toUpperCase()===plan.name?.toUpperCase()?T.terracotta:plan.popular?"rgba(250,249,245,0.08)":T.warmSand,border:`1px solid ${userPlan?.toUpperCase()===plan.name?.toUpperCase()?T.terracotta:plan.popular?T.borderDark:T.borderWarm}`,borderRadius:10,color:userPlan?.toUpperCase()===plan.name?.toUpperCase()?T.ivory:plan.popular?T.ivory:T.charcoalWarm,cursor:userPlan?.toUpperCase()===plan.name?.toUpperCase()?"default":"pointer",fontSize:15,fontFamily:"Georgia, serif",fontWeight:500}}>
                    {userPlan?.toUpperCase()===plan.name?.toUpperCase()?"✓ Current plan":"Get started →"}
                  </button>
                </div>
              ))}
            </div>

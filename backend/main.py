...
def init_db():
    ...
    for col in ["monthly_scan_count INTEGER DEFAULT 0", "monthly_reset_date TEXT",
                "api_key TEXT", "verified INTEGER DEFAULT 0", "enterprise_owner_id INTEGER DEFAULT 0"]:
...
<body style="margin:0;padding:0;background:#f5f4ed;font-family:system-ui,sans-serif">
  <div style="max-width:480px;margin:0 auto;padding:32px 16px">
    <div style="background:#141413;border-radius:16px 16px 0 0;padding:32px;text-align:center">
      <div style="font-family:Georgia,serif;font-size:24px;font-weight:500;color:#faf9f5">
        Phoenix <span style="color:#c96442">AI</span>
      </div>
      <div style="font-size:11px;color:#87867f;letter-spacing:1px;text-transform:uppercase;margin-top:4px">Email Verification</div>
    </div>
    <div style="background:#faf9f5;border:1px solid #e8e6dc;border-top:none;border-radius:0 0 16px 16px;padding:40px;text-align:center">
      <div style="font-family:Georgia,serif;font-size:20px;color:#141413;font-weight:500;margin-bottom:8px">Verify your email</div>
      <div style="font-size:14px;color:#87867f;margin-bottom:32px;line-height:1.6">Enter this OTP to complete your registration</div>
      <div style="background:#141413;border-radius:12px;padding:24px;margin-bottom:32px">
        <div style="font-family:Georgia,serif;font-size:40px;font-weight:500;color:#c96442;letter-spacing:8px">{otp}</div>
      </div>
      <div style="font-size:13px;color:#87867f;margin-bottom:8px">Expires in <strong>10 minutes</strong></div>
      <div style="font-size:12px;color:#b0aea5">If you didn't request this, ignore this email.</div>
      <div style="margin-top:32px;padding-top:20px;border-top:1px solid #e8e6dc;font-size:11px;color:#b0aea5">Phoenix AI Security Platform</div>
    </div>
  </div>
</body>
</html>"""
        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"Phoenix AI — Your verification code is {otp}"
        msg["From"] = EMAIL_SENDER
        msg["To"] = to_email
        msg.attach(MIMEText(html, "html"))
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
...
def send_email_alert(to_email: str, target: str, scan_result: dict):
    try:
        ...
        for f in findings:
            color = "#b53333" if f['severity']=="CRITICAL" else "#c96442" if f['severity']=="HIGH" else "#87867f"
            findings_rows += f"""<tr>
              <td style="padding:12px 16px;border-bottom:1px solid #f0eee6;font-size:14px;color:#141413;font-weight:500">{f['type']}</td>
              <td style="padding:12px 16px;border-bottom:1px solid #f0eee6;font-size:12px;font-weight:700;color:{color};text-transform:uppercase">{f['severity']}</td>
              <td style="padding:12px 16px;border-bottom:1px solid #f0eee6;font-size:13px;color:#5e5d59">{f['detail'][:80]}</td>
            </tr>"""
        risk_color = "#b53333" if risk > 70 else "#2d6a4f"
        html = f"""<!DOCTYPE html>
<html><body style="margin:0;padding:0;background:#f5f4ed;font-family:system-ui,sans-serif">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px">
    <div style="background:#141413;border-radius:16px 16px 0 0;padding:32px;text-align:center">
      <div style="font-family:Georgia,serif;font-size:28px;font-weight:500;color:#faf9f5">Phoenix <span style="color:#c96442">AI</span></div>
      <div style="font-size:11px;color:#87867f;letter-spacing:1px;text-transform:uppercase;margin-top:6px">Security Alert</div>
    </div>
    <div style="background:#faf9f5;border:1px solid #e8e6dc;border-top:none;border-radius:0 0 16px 16px;padding:40px">
      <div style="margin-bottom:28px">
      ...
      <table style="width:100%;border-collapse:separate;border-spacing:8px;margin-bottom:32px">
        <tr>
          <td style="background:#f5f4ed;border:1px solid #e8e6dc;border-radius:12px;padding:20px;text-align:center">
            <div style="font-size:10px;color:#87867f;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">Risk Score</div>
            <div style="font-family:Georgia,serif;font-size:32px;font-weight:500;color:{risk_color}">{risk}<span style="font-size:16px">/100</span></div>
          </td>
          <td style="background:#f5f4ed;border:1px solid #e8e6dc;border-radius:12px;padding:20px;text-align:center">
      ...
      {f'<div style="background:#141413;border-radius:12px;padding:24px;margin-bottom:28px"><div style="font-size:11px;color:#c96442;letter-spacing:0.5px;text-transform:uppercase;margin-bottom:12px">AI Analysis</div><div style="font-family:Georgia,serif;font-size:15px;color:#b0aea5;line-height:1.7;font-style:italic">{scan_result.get("ai_explanation","")}</div></div>' if scan_result.get('ai_explanation') and 'Error' not in str(scan_result.get('ai_explanation','')) else ''}
      <div style="font-size:11px;color:#87867f;letter-spacing:0.5px;text-transform:uppercase;margin-bottom:12px">Top Vulnerabilities</div>
      <table style="width:100%;border-collapse:collapse;background:#f5f4ed;border:1px solid #e8e6dc;border-radius:10px;overflow:hidden">
        <thead><tr style="background:#e8e6dc">
          <th style="padding:10px 16px;text-align:left;font-size:10px;color:#87867f;letter-spacing:1px;text-transform:uppercase;font-weight:500">Vulnerability</th>
          <th style="padding:10px 16px;text-align:left;font-size:10px;color:#87867f;letter-spacing:1px;text-transform:uppercase;font-weight:500">Severity</th>
          <th style="padding:10px 16px;text-align:left;font-size:10px;color:#87867f;letter-spacing:1px;text-transform:uppercase;font-weight:500">Detail</th>
        </tr></thead>
      ...
      <div style="margin-top:32px;padding-top:20px;border-top:1px solid #e8e6dc;text-align:center">
        ...
        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"🔴 Phoenix AI: {scan_result.get('total_findings',0)} vulnerabilities in {target}"
        msg["From"] = EMAIL_SENDER
        msg["To"] = EMAIL_SENDER
        msg.attach(MIMEText(html, "html"))
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
...
def check_sql_injection(url, findings):
    ...
    try:
        for payload in payloads:
            ...
            for error in errors:
                if error.lower() in r.text.lower():
                    findings.append({"type":"SQL Injection","severity":"CRITICAL","detail":f"SQL error with: {payload}","fix":"Use parameterized queries","poc":f"URL: {test_url}"}); return
    except: pass

def check_xss(url, findings):
    try:
        for payload in ["<script>alert('XSS')</script>","<img src=x onerror=alert(1)>"]:
            test_url = url+("&" if "?" in url else "?")+"q="+payload
            r = requests.get(test_url, timeout=6, verify=False)
            if payload in r.text:
                findings.append({"type":"XSS Vulnerability","severity":"HIGH","detail":"XSS payload reflected!","fix":"Sanitize all user inputs","poc":f"URL: {test_url}"}); return
    except: pass
...
def check_ssl(url, findings):
    try:
        ...
        findings.append({"type":"SSL Issue","severity":"HIGH","detail":"SSL/TLS config problem","fix":"Fix SSL configuration"})
    except: pass
...
def check_ports(url, findings):
    try:
        from urllib.parse import urlparse
        hostname = urlparse(url).hostname
        for port, name in {21:"FTP",22:"SSH",23:"Telnet",3306:"MySQL",5432:"PostgreSQL",6379:"Redis",27017:"MongoDB",9200:"Elasticsearch"}.items():
            try:
                s = socket.socket(); s.settimeout(0.5)
                if s.connect_ex((hostname, port)) == 0:
...
def check_open_redirect(url, findings):
    try:
        for p in ["?redirect=https://evil.com","?url=https://evil.com"]:
            r = requests.get(url+p, timeout=6, verify=False, allow_redirects=False)
            if r.status_code in [301,302] and "evil.com" in r.headers.get("Location",""):
                findings.append({"type":"Open Redirect","severity":"MEDIUM","detail":"Redirects to external URLs!","fix":"Validate redirect URLs","poc":f"Test: {url+p}"}); return
    except: pass
...
def check_source_code(code, findings):
    ...
    for pattern, severity, name, fix in patterns:
        if re.findall(pattern, code, re.IGNORECASE):
            findings.append({"type":name,"severity":severity,"detail":"Vulnerable pattern found in code","fix":fix})
...
def run_url_scan(url):
    ...
    try:
        ...
        check_security_headers(resp, findings)
        if url.startswith("http://"):
            findings.append({"type":"Insecure Protocol","severity":"HIGH","detail":"Site uses HTTP not HTTPS","fix":"Install SSL certificate"})
        if "Server" in resp.headers:
            findings.append({"type":"Server Version Disclosure","severity":"LOW","detail":f"Server: {resp.headers['Server']}","fix":"Hide server version"})
        soup = BeautifulSoup(resp.text, "html.parser")
        for i, form in enumerate(soup.find_all("form")):
            if not any("csrf" in inp.get("name","").lower() or "token" in inp.get("name","").lower() for inp in form.find_all("input")):
                findings.append({"type":"CSRF Vulnerability","severity":"HIGH","detail":f"Form #{i+1} no CSRF token","fix":"Add CSRF token to all forms"})
        check_sql_injection(url, findings)
        check_xss(url, findings)
        check_open_redirect(url, findings)
        check_sensitive_files(url, findings)
        ...
        findings.append({"type":"Scan Error","severity":"INFO","detail":str(e),"fix":"Check the URL"})
    high = sum(1 for f in findings if f["severity"] in ["HIGH","CRITICAL"])
    medium = sum(1 for f in findings if f["severity"] == "MEDIUM")
    low = sum(1 for f in findings if f["severity"] == "LOW")
    return {"status":"completed","url":url,"risk_score":min(100,(high*25)+(medium*15)+(low*5)),"total_findings":len(findings),"high":high,"medium":medium,"low":low,"findings":findings}
...
def run_threat_intel(target):
    ...
    try:
        ...
        try:
            ...
            intel["isp"] = data.get("isp","Unknown")
            if abuse_score > 50:
                findings.append({"type":"High Abuse Score","severity":"CRITICAL","detail":f"IP abuse score: {abuse_score}/100","fix":"Block immediately"})
            elif abuse_score > 10:
                findings.append({"type":"Suspicious IP","severity":"HIGH","detail":f"IP abuse score: {abuse_score}/100","fix":"Monitor closely"})
        except: pass
        try:
            r = requests.get(target, timeout=8, verify=False)
            server = r.headers.get("Server","")
        ...
        try:
            if ip and ip != hostname:
                ...
                intel["blacklisted"] = len(blacklisted) > 0
                if blacklisted:
                    findings.append({"type":"IP Blacklisted","severity":"CRITICAL","detail":f"Found in: {', '.join(blacklisted)}","fix":"Contact ISP to delist"})
        except: pass
    except Exception as e:
        findings.append({"type":"Error","severity":"INFO","detail":str(e),"fix":"Check target URL"})
    high = sum(1 for f in findings if f["severity"] in ["HIGH","CRITICAL"])
...
def run_live_app_test(url, login_url, login_email, login_password):
    ...
    try:
        ...
        for cookie in session.cookies:
            ...
            if issues: findings.append({"type":"Insecure Cookie","severity":"MEDIUM","detail":f"Cookie '{cookie.name}': {', '.join(issues)}","fix":"Set HttpOnly and Secure flags"})
        try:
            if all(session.get(url, headers=headers, timeout=3).status_code==200 for _ in range(5)):
                findings.append({"type":"No Rate Limiting","severity":"MEDIUM","detail":"No rate limiting detected","fix":"Implement rate limiting"})
        except: pass
    except Exception as e:
        findings.append({"type":"Error","severity":"INFO","detail":str(e),"fix":"Check URL"})
    high = sum(1 for f in findings if f["severity"] in ["HIGH","CRITICAL"])
...
def get_ai_explanation(scan_result):
    try:
        r = Groq(api_key=GROQ_API_KEY).chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role":"user","content":f"Security scan: URL:{scan_result.get('url')} Risk:{scan_result.get('risk_score')}/100 H:{scan_result.get('high')} M:{scan_result.get('medium')} Findings:{json.dumps(scan_result.get('findings',[])[:3])}. Give 3 lines: 1.Status 2.Critical issue 3.Recommendation"}],
            max_tokens=200)
        return r.choices[0].message.content
    except Exception as e:
...
def generate_pdf_report(scan_result, target, plan="FREE"):
    try:
        ...
        TERRA=HexColor('#c96442'); DARK=HexColor('#141413'); GRAY=HexColor('#5e5d59')
        STONE=HexColor('#87867f'); LIGHT=HexColor('#f5f4ed'); IVORY=HexColor('#faf9f5')
        SAND=HexColor('#e8e6dc'); RED=HexColor('#b53333'); GREEN=HexColor('#2d6a4f')
        WHITE=white; NAVY=HexColor('#1a1a2e'); ORANGE=HexColor('#c96442')
        ...
        h1_s = S('h1', fontSize=14, fontName='Helvetica-Bold', textColor=WHITE, leading=18)
        h2_s = S('h2', fontSize=11, fontName='Helvetica-Bold', textColor=DARK, leading=15, spaceAfter=4, spaceBefore=8)
        body_s = S('body', fontSize=9, textColor=GRAY, leading=13, spaceAfter=3)
        body_j = S('bodyj', fontSize=9, textColor=GRAY, leading=13, spaceAfter=3, alignment=TA_JUSTIFY)
        label_s = S('label', fontSize=8, fontName='Helvetica-Bold', textColor=STONE, leading=11, spaceAfter=2)
        small_s = S('small', fontSize=8, textColor=STONE, leading=11)
        code_s = S('code', fontSize=8, fontName='Courier', textColor=GRAY, leading=11)
        footer_s = S('footer', fontSize=8, textColor=STONE, alignment=TA_CENTER, leading=11)
        cover_label = S('clabel', fontSize=10, textColor=HexColor('#b0aea5'), alignment=TA_CENTER, leading=14)
        cover_value = S('cvalue', fontSize=12, fontName='Helvetica-Bold', textColor=WHITE, alignment=TA_CENTER, leading=16, spaceAfter=8)
        conf_s = S('conf', fontSize=9, textColor=HexColor('#ff6600'), alignment=TA_CENTER, fontName='Helvetica-Bold', leading=12)
        ...
        sev_colors = {'CRITICAL':HexColor('#b53333'),'HIGH':HexColor('#c96442'),'MEDIUM':HexColor('#d4920a'),'LOW':HexColor('#2d6a4f'),'INFO':HexColor('#5e5d59')}
        sev_bgs = {'CRITICAL':HexColor('#fef2f2'),'HIGH':HexColor('#fff7f0'),'MEDIUM':HexColor('#fffbeb'),'LOW':HexColor('#f0fdf4'),'INFO':LIGHT}

        def section_header(title):
            t = Table([[Paragraph(title, h1_s)]], colWidths=[PAGE_W])
            t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),DARK),('PADDING',(0,0),(-1,-1),10)]))
            return t

        def info_table(data):
            rows = [[Paragraph(str(r[0]), label_s), Paragraph(str(r[1]), body_s)] for r in data]
            t = Table(rows, colWidths=[PAGE_W*0.3, PAGE_W*0.7])
            t.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),7),('VALIGN',(0,0),(-1,-1),'TOP'),('ROWBACKGROUNDS',(0,0),(-1,-1),[WHITE,LIGHT]),('FONTSIZE',(0,0),(-1,-1),9)]))
            return t
        ...
        # COVER
        cover_data = [
            [Paragraph("", cover_label)],[Paragraph("", cover_label)],[Paragraph("", cover_label)],
            [Paragraph("Phoenix AI", S('logo', fontSize=32, fontName='Helvetica-Bold', textColor=TERRA, alignment=TA_CENTER, leading=38))],
            [Paragraph("WEB APPLICATION PENETRATION TESTING REPORT", S('ct', fontSize=14, fontName='Helvetica-Bold', textColor=WHITE, alignment=TA_CENTER, leading=18))],
            [Paragraph("", cover_label)],
            [Paragraph("─" * 60, S('div', fontSize=8, textColor=HexColor('#444'), alignment=TA_CENTER))],
            [Paragraph("", cover_label)],
            [Paragraph("Client / Target", cover_label)],
        ...
        ct = Table(cover_data, colWidths=[PAGE_W])
        ct.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),NAVY),('PADDING',(0,0),(-1,-1),6),('TOPPADDING',(0,0),(0,0),40),('BOTTOMPADDING',(0,-1),(0,-1),40)]))
        story.append(ct)
        story.append(PageBreak())
        ...
        story.append(section_header("DOCUMENT CONTROL"))
        story.append(Spacer(1,8))
        dc = Table([["Version","Date","Prepared By","Description"],["1.0",now_str,"Phoenix AI","Initial Security Report"]], colWidths=[PAGE_W*0.12,PAGE_W*0.22,PAGE_W*0.3,PAGE_W*0.36])
        dc.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),DARK),('TEXTCOLOR',(0,0),(-1,0),WHITE),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),9),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),7),('ROWBACKGROUNDS',(0,1),(-1,-1),[WHITE,LIGHT])]))
        story.append(dc)
        story.append(Spacer(1,12))
        story.append(Paragraph("CONFIDENTIALITY NOTICE", h2_s))
        story.append(Paragraph("This document contains confidential and proprietary information. The information provided in this report is intended solely for the authorized personnel of the client organization. Unauthorized disclosure, copying, or distribution of this document is strictly prohibited.", body_j))
        story.append(Spacer(1,8))

        # TOC
        story.append(section_header("TABLE OF CONTENTS"))
        story.append(Spacer(1,8))
        toc_items = [("1.","Executive Summary"),("2.","Objective"),("3.","Scope of Assessment"),("4.","Assessment Methodology"),("5.","Tools Used"),("6.","Risk Rating Methodology"),("7.","Summary of Findings"),("8.","Detailed Vulnerability Findings"),("9.","Positive Security Observations"),("10.","Recommendations"),("11.","Conclusion"),("12.","Appendix")]
        toc_t = Table([[Paragraph(n, S('tn', fontSize=9, fontName='Helvetica-Bold', textColor=TERRA)), Paragraph(t, toc_s)] for n,t in toc_items], colWidths=[PAGE_W*0.08, PAGE_W*0.92])
        toc_t.setStyle(TableStyle([('PADDING',(0,0),(-1,-1),5),('LINEBELOW',(0,0),(-1,-2),0.3,SAND)]))
        story.append(toc_t)
        story.append(PageBreak())
        ...
        story.append(Spacer(1,8))
        story.append(Paragraph("Overview", h2_s))
        story.append(Paragraph(f"A security assessment was conducted on <b>{target}</b> to identify vulnerabilities that could impact the confidentiality, integrity, and availability of the system. The assessment included manual and automated testing techniques based on industry-standard methodologies.", body_j))
        story.append(Spacer(1,8))
        story.append(Paragraph("Risk Summary", h2_s))
        critical_c = sum(1 for f in findings if f.get('severity')=='CRITICAL')
        info_c = sum(1 for f in findings if f.get('severity')=='INFO')
        rs_data = [["Severity","Count","Status"],["Critical",str(critical_c),"Open" if critical_c>0 else "None"],["High",str(high_count),"Open" if high_count>0 else "None"],["Medium",str(medium_count),"Open" if medium_count>0 else "None"],["Low",str(low_count),"Open" if low_count>0 else "None"],["Informational",str(info_c),"Noted"]]
        rs_t = Table(rs_data, colWidths=[PAGE_W*0.33,PAGE_W*0.33,PAGE_W*0.34])
        rs_cmds = [('BACKGROUND',(0,0),(-1,0),DARK),('TEXTCOLOR',(0,0),(-1,0),WHITE),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),8),('ALIGN',(1,0),(2,-1),'CENTER'),('FONTSIZE',(0,0),(-1,-1),9)]
        for i,bg,tc in [(1,HexColor('#fef2f2'),HexColor('#b53333')),(2,HexColor('#fff7f0'),HexColor('#c96442')),(3,HexColor('#fffbeb'),HexColor('#d4920a')),(4,HexColor('#f0fdf4'),HexColor('#2d6a4f')),(5,LIGHT,GRAY)]:
            rs_cmds += [('BACKGROUND',(0,i),(-1,i),bg),('TEXTCOLOR',(0,i),(0,i),tc),('FONTNAME',(0,i),(0,i),'Helvetica-Bold')]
        rs_t.setStyle(TableStyle(rs_cmds))
        story.append(rs_t)
        story.append(Spacer(1,8))

        overall = "CRITICAL" if risk>70 else "HIGH" if risk>50 else "MEDIUM" if risk>30 else "LOW"
        risk_col = RED if risk>70 else ORANGE if risk>50 else HexColor('#d4920a') if risk>30 else GREEN
        or_t = Table([[Paragraph("Overall Risk Level", S('orl', fontSize=10, fontName='Helvetica-Bold', textColor=WHITE)), Paragraph(f"{overall}  ({risk}/100)", S('orv', fontSize=12, fontName='Helvetica-Bold', textColor=risk_col, alignment=TA_RIGHT))]], colWidths=[PAGE_W*0.5,PAGE_W*0.5])
        or_t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),DARK),('PADDING',(0,0),(-1,-1),10)]))
        story.append(or_t)
        story.append(Spacer(1,8))

        if scan_result.get('ai_explanation') and 'Error' not in str(scan_result.get('ai_explanation','')):
            story.append(Paragraph("AI Security Analysis", h2_s))
            ai_box = Table([[Paragraph(scan_result['ai_explanation'], body_j)]], colWidths=[PAGE_W])
            ai_box.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),LIGHT),('LEFTPADDING',(0,0),(-1,-1),14),('RIGHTPADDING',(0,0),(-1,-1),14),('TOPPADDING',(0,0),(-1,-1),10),('BOTTOMPADDING',(0,0),(-1,-1),10),('LINERIGHT',(0,0),(0,-1),3,TERRA)]))
            story.append(ai_box)
        story.append(Spacer(1,8))
        ...
        story.append(section_header("2. OBJECTIVE"))
        story.append(Spacer(1,8))
        story.append(Paragraph("The objective of this assessment was to identify security weaknesses in the target application and provide remediation recommendations to improve the overall security posture.", body_j))
        story.append(Spacer(1,8))
        ...
        story.append(Paragraph("Target Information", h2_s))
        intel = scan_result.get('intel', {})
        story.append(info_table([["Target URL",target],["IP Address",intel.get('ip','N/A') if intel else 'N/A'],["Application Type","Web Application"],["Testing Type","Black Box"],["Environment","Production"],["Assessment Date",now_full],["Risk Score",f"{risk}/100"],["Plan",plan]]))
        story.append(Spacer(1,8))

        # 4. METHODOLOGY
        story.append(section_header("4. ASSESSMENT METHODOLOGY"))
        story.append(Spacer(1,8))
        phases = [["4.1","Reconnaissance","Information gathering, subdomain enumeration, technology identification"],["4.2","Scanning","Port scanning, security header analysis, SSL/TLS validation"],["4.3","Vulnerability Assessment","Automated and manual vulnerability identification"],["4.4","Exploitation","Controlled exploitation and PoC validation"],["4.5","Reporting","Documentation, risk classification, remediation guidance"]]
        ph_t = Table(phases, colWidths=[PAGE_W*0.08,PAGE_W*0.3,PAGE_W*0.62])
        ph_t.setStyle(TableStyle([('FONTSIZE',(0,0),(-1,-1),9),('FONTNAME',(0,0),(0,-1),'Helvetica-Bold'),('TEXTCOLOR',(0,0),(0,-1),TERRA),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),7),('ROWBACKGROUNDS',(0,0),(-1,-1),[WHITE,LIGHT]),('VALIGN',(0,0),(-1,-1),'TOP')]))
        story.append(ph_t)
        story.append(Spacer(1,8))
        ...
        story.append(section_header("5. TOOLS USED"))
        story.append(Spacer(1,8))
        tools = [["Tool","Purpose"],["Phoenix AI Scanner","Automated Web Application Testing"],["Custom Python Scripts","SSL/TLS, Header & Port Analysis"],["AbuseIPDB API","IP Reputation & Threat Intelligence"],["Groq AI (LLaMA)","AI-Powered Vulnerability Analysis"],["DNS Resolver","Subdomain Enumeration"],["Socket Scanner","Port & Service Detection"]]
        tools_t = Table(tools, colWidths=[PAGE_W*0.4,PAGE_W*0.6])
        tools_t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),DARK),('TEXTCOLOR',(0,0),(-1,0),WHITE),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),9),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),7),('ROWBACKGROUNDS',(0,1),(-1,-1),[WHITE,LIGHT])]))
        story.append(tools_t)
        story.append(Spacer(1,8))
        ...
        story.append(section_header("6. RISK RATING METHODOLOGY"))
        story.append(Spacer(1,8))
        rr = [["Severity","CVSS Score","Description"],["Critical","9.0–10.0","Immediate exploitation risk — urgent remediation"],["High","7.0–8.9","Significant risk — fix within 7 days"],["Medium","4.0–6.9","Moderate risk — fix within 30 days"],["Low","0.1–3.9","Minor risk — fix within 90 days"],["Informational","0.0","No direct impact"]]
        rr_t = Table(rr, colWidths=[PAGE_W*0.22,PAGE_W*0.22,PAGE_W*0.56])
        rr_cmds = [('BACKGROUND',(0,0),(-1,0),DARK),('TEXTCOLOR',(0,0),(-1,0),WHITE),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),9),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),8)]
        for i,bg,tc in [(1,HexColor('#fef2f2'),HexColor('#b53333')),(2,HexColor('#fff7f0'),HexColor('#c96442')),(3,HexColor('#fffbeb'),HexColor('#d4920a')),(4,HexColor('#f0fdf4'),HexColor('#2d6a4f')),(5,LIGHT,GRAY)]:
            rr_cmds += [('BACKGROUND',(0,i),(-1,i),bg),('TEXTCOLOR',(0,i),(0,i),tc),('FONTNAME',(0,i),(0,i),'Helvetica-Bold')]
        rr_t.setStyle(TableStyle(rr_cmds))
        story.append(rr_t)
        story.append(Spacer(1,8))
        ...
        sum_data = [["ID","Vulnerability","Severity","Status"]]
        for i,f in enumerate(findings):
            sum_data.append([f"V-{str(i+1).zfill(2)}",f.get('type','Unknown'),f.get('severity','INFO'),"Open"])
        sum_t = Table(sum_data, colWidths=[PAGE_W*0.1,PAGE_W*0.5,PAGE_W*0.2,PAGE_W*0.2])
        sum_cmds = [('BACKGROUND',(0,0),(-1,0),DARK),('TEXTCOLOR',(0,0),(-1,0),WHITE),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),9),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),7),('ROWBACKGROUNDS',(0,1),(-1,-1),[WHITE,LIGHT])]
        for i,f in enumerate(findings,1):
            col = sev_colors.get(f.get('severity','INFO'),GRAY)
            sum_cmds += [('TEXTCOLOR',(2,i),(2,i),col),('FONTNAME',(2,i),(2,i),'Helvetica-Bold')]
        sum_t.setStyle(TableStyle(sum_cmds))
        story.append(sum_t)
        story.append(PageBreak())
        ...
        for i,f in enumerate(findings):
            sev = f.get('severity','INFO')
            sev_col = sev_colors.get(sev,GRAY)
            sev_bg = sev_bgs.get(sev,LIGHT)
            vuln_id = f"V-{str(i+1).zfill(2)}"
            vh_t = Table([[Paragraph(f"{vuln_id}  {f.get('type','')}", S('vht', fontSize=11, fontName='Helvetica-Bold', textColor=WHITE)),Paragraph(sev, S('vhs', fontSize=10, fontName='Helvetica-Bold', textColor=sev_col, alignment=TA_RIGHT))]], colWidths=[PAGE_W*0.75,PAGE_W*0.25])
            vh_t.setStyle(TableStyle([('BACKGROUND',(0,0),(0,0),DARK),('BACKGROUND',(1,0),(1,0),sev_bg),('PADDING',(0,0),(-1,-1),10)]))
            detail_rows = [
                [Paragraph("<b>Severity</b>", label_s), Paragraph(sev, body_s)],
                [Paragraph("<b>Target</b>", label_s), Paragraph(str(target), body_s)],
                [Paragraph("<b>Description</b>", label_s), Paragraph(str(f.get('detail','N/A')), body_s)],
                [Paragraph("<b>Impact</b>", label_s), Paragraph(f"This vulnerability may allow attackers to compromise {target}. Immediate attention recommended.", body_s)],
                [Paragraph("<b>Remediation</b>", label_s), Paragraph(str(f.get('fix','N/A')), body_s)],
            ]
            if f.get('poc') and plan in ["PRO","ENTERPRISE"]:
                detail_rows.append([Paragraph("<b>Proof of Concept</b>", label_s), Paragraph(str(f.get('poc','')), code_s)])
            if f.get('file'):
                detail_rows.append([Paragraph("<b>File</b>", label_s), Paragraph(str(f.get('file','')), code_s)])
            dt = Table(detail_rows, colWidths=[PAGE_W*0.25,PAGE_W*0.75])
            dt.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),8),('VALIGN',(0,0),(-1,-1),'TOP'),('ROWBACKGROUNDS',(0,0),(-1,-1),[WHITE,LIGHT]),('FONTSIZE',(0,0),(-1,-1),9)]))
            story.append(KeepTogether([vh_t, dt]))
            story.append(Spacer(1,12))
        ...
        if not any('Redirect' in t for t in all_types): positive.append("No open redirects detected")
        if not any('Port' in t for t in all_types): positive.append("No dangerous ports exposed")
        positive += ["HTTPS protocol in use","Application is accessible and functional"]
        pos_t = Table([[Paragraph("✓", S('pg', fontSize=10, fontName='Helvetica-Bold', textColor=GREEN)), Paragraph(p, body_s)] for p in positive], colWidths=[PAGE_W*0.06,PAGE_W*0.94])
        pos_t.setStyle(TableStyle([('PADDING',(0,0),(-1,-1),6),('LINEBELOW',(0,0),(-1,-2),0.3,SAND)]))
        story.append(pos_t)
        story.append(Spacer(1,8))
        ...
        story.append(section_header("10. RECOMMENDATIONS"))
        story.append(Spacer(1,8))
        story.append(Paragraph("Immediate Actions", h2_s))
        imm_t = Table([["1.","Fix all Critical and High severity vulnerabilities immediately"],["2.","Implement missing security headers (CSP, HSTS, X-Frame-Options)"],["3.","Add CSRF tokens to all forms"],["4.","Patch outdated software"],["5.","Disable unnecessary exposed ports"]], colWidths=[PAGE_W*0.06,PAGE_W*0.94])
        imm_t.setStyle(TableStyle([('FONTSIZE',(0,0),(-1,-1),9),('TEXTCOLOR',(0,0),(0,-1),TERRA),('FONTNAME',(0,0),(0,-1),'Helvetica-Bold'),('PADDING',(0,0),(-1,-1),5),('LINEBELOW',(0,0),(-1,-2),0.3,SAND)]))
        story.append(imm_t)
        story.append(Spacer(1,8))
        ...
        story.append(section_header("11. CONCLUSION"))
        story.append(Spacer(1,8))
        story.append(Paragraph(f"The security assessment of <b>{target}</b> identified <b>{total} vulnerabilities</b> with an overall risk score of <b>{risk}/100</b> ({overall} risk level). Immediate remediation of critical and high-risk issues is strongly recommended.", body_j))
        story.append(Spacer(1,8))
        ...
        story.append(Spacer(1,8))
        story.append(Paragraph("Appendix A — Scan Configuration", h2_s))
        app_a = Table([["Parameter","Value"],["Target URL",target],["Scan Type",scan_result.get('scan_type','URL').upper()],["Scan Date",now_full],["Plan",plan]], colWidths=[PAGE_W*0.35,PAGE_W*0.65])
        app_a.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),DARK),('TEXTCOLOR',(0,0),(-1,0),WHITE),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),('FONTSIZE',(0,0),(-1,-1),9),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),7),('ROWBACKGROUNDS',(0,1),(-1,-1),[WHITE,LIGHT])]))
        story.append(app_a)
        story.append(Spacer(1,8))

        if intel:
            story.append(Paragraph("Appendix B — Threat Intelligence", h2_s))
            intel_rows = [[k.replace('_',' ').title(), str(v)] for k,v in intel.items() if k != 'subdomains' and v is not None]
            if intel_rows:
                ia_t = Table(intel_rows, colWidths=[PAGE_W*0.35,PAGE_W*0.65])
                ia_t.setStyle(TableStyle([('FONTSIZE',(0,0),(-1,-1),9),('FONTNAME',(0,0),(0,-1),'Helvetica-Bold'),('GRID',(0,0),(-1,-1),0.5,SAND),('PADDING',(0,0),(-1,-1),6),('ROWBACKGROUNDS',(0,0),(-1,-1),[WHITE,LIGHT])]))
                story.append(ia_t)
            story.append(Spacer(1,8))
        ...
        story.append(HRFlowable(width="100%", thickness=1, color=SAND))
        story.append(Spacer(1,8))
        ft = Table([[Paragraph("Prepared By:\nGlobal SecureLayer X Pvt. Ltd.", footer_s),Paragraph("Phoenix AI Security Platform", S('fc', fontSize=9, textColor=TERRA, alignment=TA_CENTER, fontName='Helvetica-Bold')),Paragraph("Email: securelayerhelp@gmail.com\nClassification: CONFIDENTIAL", footer_s)]], colWidths=[PAGE_W*0.33,PAGE_W*0.34,PAGE_W*0.33])
        ft.setStyle(TableStyle([('PADDING',(0,0),(-1,-1),6),('VALIGN',(0,0),(-1,-1),'TOP')]))
        story.append(ft)
...
def register(user: UserRegister):
    conn = sqlite3.connect("voidscan.db")
    c = conn.cursor()
    c.execute("SELECT id, verified FROM users WHERE email=?", (user.email,))
    existing = c.fetchone()
    if existing:
        if existing[1] == 1:
            conn.close()
            ...
            c.execute("DELETE FROM email_verifications WHERE email=?", (user.email,))
            c.execute("INSERT INTO email_verifications (email, otp, created_at, expires_at) VALUES (?, ?, ?, ?)", (user.email, otp, datetime.now().isoformat(), expires))
            conn.commit(); conn.close()
            send_verification_email(user.email, otp)
            return {"message": "OTP_SENT", "email": user.email}
    hashed = hashlib.sha256(user.password.encode()).hexdigest()
    try:
        c.execute("INSERT INTO users (email, password, plan, scan_count, monthly_scan_count, created_at, verified) VALUES (?, ?, 'FREE', 0, 0, ?, 0)", (user.email, hashed, datetime.now().isoformat()))
        conn.commit()
        otp = generate_otp()
        expires = (datetime.now() + timedelta(minutes=10)).isoformat()
        c.execute("INSERT INTO email_verifications (email, otp, created_at, expires_at) VALUES (?, ?, ?, ?)", (user.email, otp, datetime.now().isoformat(), expires))
...
def verify_otp(data: VerifyOTP):
    conn = sqlite3.connect("voidscan.db")
    c = conn.cursor()
    c.execute("SELECT otp, expires_at, verified FROM email_verifications WHERE email=? ORDER BY id DESC LIMIT 1", (data.email,))
    row = c.fetchone()
    if not row: conn.close(); raise HTTPException(status_code=400, detail="OTP not found!")
    if row[2] == 1: conn.close(); raise HTTPException(status_code=400, detail="Already verified!")
    if datetime.fromisoformat(row[1]) < datetime.now(): conn.close(); raise HTTPException(status_code=400, detail="OTP expired!")
    if row[0] != data.otp: conn.close(); raise HTTPException(status_code=400, detail="Invalid OTP!")
    c.execute("UPDATE users SET verified=1 WHERE email=?", (data.email,))
    c.execute("UPDATE email_verifications SET verified=1 WHERE email=?", (data.email,))
    conn.commit()
    c.execute("SELECT id, email, plan, monthly_scan_count FROM users WHERE email=?", (data.email,))
    user = c.fetchone()
    conn.close()
    return {"message": "Email verified!", "user_id": user[0], "email": user[1], "plan": user[2], "monthly_scan_count": user[3] or 0}
...
def resend_otp(data: ResendOTP):
    ...
    if not c.fetchone(): conn.close(); raise HTTPException(status_code=404, detail="Email not found!")
    otp = generate_otp()
    expires = (datetime.now() + timedelta(minutes=10)).isoformat()
    c.execute("DELETE FROM email_verifications WHERE email=?", (data.email,))
    c.execute("INSERT INTO email_verifications (email, otp, created_at, expires_at) VALUES (?, ?, ?, ?)", (data.email, otp, datetime.now().isoformat(), expires))
    conn.commit(); conn.close()
...
def login(user: UserLogin):
    ...
    c = conn.cursor()
    hashed = hashlib.sha256(user.password.encode()).hexdigest()
    c.execute("SELECT id, email, plan, monthly_scan_count, monthly_reset_date, plan_expires, verified FROM users WHERE email=? AND password=?", (user.email, hashed))
    db_user = c.fetchone()
    if not db_user: conn.close(); raise HTTPException(status_code=401, detail="Invalid credentials!")
    if db_user[6] == 0: conn.close(); raise HTTPException(status_code=403, detail="EMAIL_NOT_VERIFIED")
    now = datetime.now()
    monthly_count = db_user[3] or 0
    reset_date = db_user[4]
    if reset_date:
    ...
    c.execute("UPDATE users SET last_login=? WHERE id=?", (now.isoformat(), db_user[0]))
    conn.commit(); conn.close()
    return {"message":"Login successful!","email":db_user[1],"user_id":db_user[0],"plan":db_user[2]or"FREE","monthly_scan_count":monthly_count,"plan_expires":db_user[5]}

@app.post("/scan")
def create_scan(scan: ScanRequest):
    PLAN_LIMITS = {"FREE":3,"PRO":-1,"ENTERPRISE":-1}
    FREE_SCANNERS = ["url"]
    user_plan = "FREE"; monthly_count = 0
    if scan.user_id:
        conn = sqlite3.connect("voidscan.db")
        c = conn.cursor()
        c.execute("SELECT plan, monthly_scan_count, monthly_reset_date FROM users WHERE id=?", (scan.user_id,))
        ...
        conn.close()
        if row:
            user_plan = row[0] or "FREE"; monthly_count = row[1] or 0; reset_date = row[2]
            now = datetime.now()
            if reset_date:
                try:
                    reset_dt = datetime.fromisoformat(reset_date)
                    if now.month != reset_dt.month or now.year != reset_dt.year:
                        monthly_count = 0
                        conn2 = sqlite3.connect("voidscan.db"); c2 = conn2.cursor()
                        c2.execute("UPDATE users SET monthly_scan_count=0, monthly_reset_date=? WHERE id=?", (now.isoformat(), scan.user_id))
                        conn2.commit(); conn2.close()
                except: pass
            limit = PLAN_LIMITS.get(user_plan, 3)
    ...
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("INSERT INTO scans (user_id, target, scan_type, status, result, created_at) VALUES (?, ?, ?, ?, ?, ?)", (scan.user_id, scan.target, scan.scan_type, "completed", json.dumps(result), datetime.now().isoformat()))
    conn.commit(); scan_id = c.lastrowid
    if scan.user_id:
        c.execute("UPDATE users SET scan_count=scan_count+1, monthly_scan_count=monthly_scan_count+1, monthly_reset_date=COALESCE(monthly_reset_date,?) WHERE id=?", (datetime.now().isoformat(), scan.user_id))
        conn.commit()
    conn.close()
    try: send_email_alert(EMAIL_SENDER, scan.target, result)
    except: pass
...
async def scan_zip(file: UploadFile = File(...)):
    try:
        ...
        os.unlink(tmp_path)
        conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
        c.execute("INSERT INTO scans (target, scan_type, status, result, created_at) VALUES (?, ?, ?, ?, ?)", (file.filename, "zip", "completed", json.dumps(result), datetime.now().isoformat()))
        conn.commit(); scan_id = c.lastrowid; conn.close()
        return {"scan_id": scan_id, "result": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def download_pdf(scan_id: int):
    ...
    return Response(content=pdf, media_type="application/pdf", headers={"Content-Disposition": f"attachment; filename=phoenix-ai-report-{scan_id}.pdf"})
...
def invite_team_member(data: InviteTeamMember):
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("SELECT plan FROM users WHERE id=?", (data.owner_id,))
    owner = c.fetchone()
    if not owner or owner[0] != "ENTERPRISE":
        conn.close(); raise HTTPException(status_code=403, detail="Enterprise plan required!")
    c.execute("SELECT COUNT(*) FROM team_members WHERE enterprise_owner_id=? AND status='active'", (data.owner_id,))
    if c.fetchone()[0] >= 5:
        conn.close(); raise HTTPException(status_code=400, detail="Team limit reached (5 max)!")
    c.execute("SELECT id FROM team_members WHERE enterprise_owner_id=? AND member_email=?", (data.owner_id, data.member_email))
    if c.fetchone():
        conn.close(); raise HTTPException(status_code=400, detail="Already invited!")
    c.execute("INSERT INTO team_members (enterprise_owner_id, member_email, status, invited_at) VALUES (?, ?, 'pending', ?)", (data.owner_id, data.member_email, datetime.now().isoformat()))
    conn.commit(); conn.close()
    try:
        html = f"""<div style="font-family:system-ui;max-width:480px;margin:0 auto;padding:32px">
          <div style="background:#141413;border-radius:16px;padding:32px;text-align:center">
            <div style="font-family:Georgia,serif;font-size:24px;color:#faf9f5">Phoenix <span style="color:#c96442">AI</span></div>
          </div>
          <div style="background:#faf9f5;border:1px solid #e8e6dc;padding:32px;border-radius:0 0 16px 16px">
            <h2 style="font-family:Georgia,serif;color:#141413">You've been invited!</h2>
            <p style="color:#5e5d59">You've been invited to join an Enterprise team on Phoenix AI.</p>
            <p style="color:#5e5d59">Register with this email address to join automatically.</p>
          </div>
        </div>"""
        msg = MIMEMultipart("alternative")
        msg["Subject"] = "You've been invited to Phoenix AI Enterprise"
        msg["From"] = EMAIL_SENDER
        msg["To"] = data.member_email
        msg.attach(MIMEText(html, "html"))
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
            server.login(EMAIL_SENDER, EMAIL_PASSWORD)
            server.sendmail(EMAIL_SENDER, data.member_email, msg.as_string())
    except: pass
    return {"message": f"Invitation sent to {data.member_email}"}

@app.get("/team/members/{owner_id}")
def get_team_members(owner_id: int):
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("SELECT id, member_email, status, invited_at FROM team_members WHERE enterprise_owner_id=?", (owner_id,))
    rows = c.fetchall(); conn.close()
    return {"members": [{"id":r[0],"email":r[1],"status":r[2],"invited_at":r[3]} for r in rows]}

@app.delete("/team/remove/{member_id}")
def remove_team_member(member_id: int, owner_id: int):
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("DELETE FROM team_members WHERE id=? AND enterprise_owner_id=?", (member_id, owner_id))
    conn.commit(); conn.close()
...
def generate_api_key_route(data: GenerateAPIKey):
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("SELECT plan FROM users WHERE id=?", (data.user_id,))
    user = c.fetchone()
    if not user or user[0] != "ENTERPRISE":
        conn.close(); raise HTTPException(status_code=403, detail="Enterprise plan required!")
    api_key = "pai_" + secrets.token_hex(24)
    c.execute("UPDATE users SET api_key=? WHERE id=?", (api_key, data.user_id))
    conn.commit(); conn.close()
    return {"api_key": api_key, "message": "API key generated!"}

@app.get("/api-key/{user_id}")
def get_api_key(user_id: int):
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("SELECT api_key, plan FROM users WHERE id=?", (user_id,))
    row = c.fetchone(); conn.close()
    if not row or row[1] != "ENTERPRISE":
        raise HTTPException(status_code=403, detail="Enterprise plan required!")
    return {"api_key": row[0] or None}
...
def admin_users(secret: str):
    if secret != ADMIN_SECRET: raise HTTPException(status_code=403, detail="Unauthorized")
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("SELECT id, email, plan, scan_count, monthly_scan_count, plan_expires, last_login, created_at, verified FROM users ORDER BY id DESC")
    rows = c.fetchall(); conn.close()
    return {"users": [{"id":r[0],"email":r[1],"plan":r[2]or"FREE","scan_count":r[3]or 0,"monthly_scan_count":r[4]or 0,"plan_expires":r[5],"last_login":r[6],"created_at":r[7],"verified":r[8]or 0} for r in rows]}

@app.post("/admin/update-plan")
def update_plan(data: UpdatePlan):
    if data.admin_secret != ADMIN_SECRET: raise HTTPException(status_code=403, detail="Unauthorized")
    expires = None
    if data.plan != "FREE": expires = (datetime.now() + timedelta(days=30)).isoformat()
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("UPDATE users SET plan=?, plan_started=?, plan_expires=?, verified=? WHERE id=?", (data.plan, datetime.now().isoformat(), expires, 1 if data.plan in ["PRO","ENTERPRISE"] else 0, data.user_id))
    conn.commit(); conn.close()
    return {"message": f"Plan updated to {data.plan}", "expires": expires}
...
def admin_scans(secret: str):
    if secret != ADMIN_SECRET: raise HTTPException(status_code=403, detail="Unauthorized")
    conn = sqlite3.connect("voidscan.db"); c = conn.cursor()
    c.execute("SELECT s.id, s.target, s.scan_type, s.created_at, u.email FROM scans s LEFT JOIN users u ON s.user_id = u.id ORDER BY s.id DESC LIMIT 100")
    rows = c.fetchall(); conn.close()
    return {"scans": [{"id":r[0],"target":r[1],"type":r[2],"date":r[3],"user":r[4]or"Anonymous"} for r in rows]}
...
def delete_user(user_id: int, secret: str):
    ...
    c.execute("DELETE FROM users WHERE id=?", (user_id,))
    c.execute("DELETE FROM scans WHERE user_id=?", (user_id,))
    c.execute("DELETE FROM team_members WHERE enterprise_owner_id=? OR member_user_id=?", (user_id, user_id))
    conn.commit(); conn.close()
